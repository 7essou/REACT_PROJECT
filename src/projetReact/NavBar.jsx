...
export default function NavBar(){
    ...
    return  <nav className="flex  gap-4 relative shadow-md   w-full p-2 pb-0 mb-1  border-b border-neutral-200 ">
        ...
        <ul className=" grid grid-cols-3 gap-1 mt-2 md:ml-30    " onClick={()=>{seturl(window.location.pathname)}}>
         <li className={`md:w-30 transition-all ease-out  hover:border-b-4 hover:text-[#EEB866FF] hover:font-bold   ${(url=='/' || url==`/${undefined} ` ||url==`/${iduser}` )?'border-b-4 font-bold text-[#EEB866FF]':''}  border-[#EEB866FF] text-center `}><Link to={`/${iduser}`}>Accuel</Link></li>
